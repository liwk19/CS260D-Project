...
from train import train_main, inference, ensemble_inference
...
if __name__ == '__main__':
    ...
    if FLAGS.subtask == 'vis+inf':
        ...
        for ind, r in enumerate(resample_list):
            ...
            if FLAGS.finetune == False or FLAGS.coreset not in ['ensemble_train', 'ensemble_inference']:
                test_data = train_main(dataset, pragma_dim, test_ratio=test_ratio, resample=r)
            elif FLAGS.coreset == 'ensemble_train':
                for e_kernel in FLAGS.current_ensemble_KERNEL:
                    test_data = train_main(dataset, pragma_dim, FLAGS.val_ratio, test_ratio, r, e_kernel)
            else:
                assert FLAGS.coreset == 'ensemble_inference'
                test_data = ensemble_inference(dataset, pragma_dim, test_ratio=test_ratio, resample=r)

...
from coreset import craig_split, uncertainty_split, find_uncertain_indices
...
def split_dataset(dataset, train, val, dataset_test=None, pragma_dim=None, model=None, 
        current_train=[], num_features=None, edge_dim=None):
    file_li = dataset.processed_file_names
    if FLAGS.coreset == 'random' or FLAGS.finetune == False:
        ...
        if FLAGS.uncertainty_metric == 'False':
            li = random_split(file_li, [train, len(dataset) - train],
                            generator=torch.Generator().manual_seed(FLAGS.random_seed))
            li = [li[0], [], li[1]]
        else:
            try:
                if FLAGS.uncertainty_metric != 'hybrid':
                    split = np.load(f'/home/weikaili/software-gnn/src/ensemble_data_split/{ALL_KERNEL[0]}_{FLAGS.uncertainty_metric}.npy')
                else:
                    split_path = f'/home/weikaili/software-gnn/src/ensemble_data_split/{ALL_KERNEL[0]}_{FLAGS.uncertainty_metric}_'
                    split_path += f'{FLAGS.uncertainty_split[0]}+{FLAGS.uncertainty_split[0]}_{FLAGS.uncertainty_split_alternating}.npy'
                    split = np.load(split_path)
                li = [split[:FLAGS.transfer_k_shot], [], split[FLAGS.transfer_k_shot:]]
            except:
                uncertain_indices = find_uncertain_indices(dataset, num_features, edge_dim, pragma_dim)
                li = uncertainty_split(file_li, [train, len(dataset) - train], uncertain_indices=uncertain_indices)
                os.makedirs('/home/weikaili/software-gnn/src/ensemble_data_split', exist_ok=True)
                #os.makedirs('ensemble_data_split', exist_ok=True)
                if FLAGS.uncertainty_metric != 'hybrid':
                    np.save(f'/home/weikaili/software-gnn/src/ensemble_data_split/{ALL_KERNEL[0]}_{FLAGS.uncertainty_metric}.npy', li[0] + li[1])
                    #np.save(f'ensemble_data_split/{ALL_KERNEL[0]}_{FLAGS.uncertainty_metric}.npy', li[0] + li[1])
                else:
                    #split_path = f'ensemble_data_split/{ALL_KERNEL[0]}_{FLAGS.uncertainty_metric}_'
                    split_path = f'/home/weikaili/software-gnn/src/ensemble_data_split/{ALL_KERNEL[0]}_{FLAGS.uncertainty_metric}_'
                    split_path += f'{FLAGS.uncertainty_split[0]}+{FLAGS.uncertainty_split[0]}_{FLAGS.uncertainty_split_alternating}.npy'
                    np.save(split_path, li[0] + li[1])
                li = [li[0], [], li[1]]
    elif FLAGS.coreset == 'ensemble_inference':
        split = np.load(f'/home/weikaili/software-gnn/src/ensemble_data_split/{ALL_KERNEL[0]}_{FLAGS.uncertainty_metric}.npy')
        li = [split[:FLAGS.transfer_k_shot], [], split[FLAGS.transfer_k_shot:]]
    else:
        assert FLAGS.coreset == 'CRAIG'
        if FLAGS.craig_metric == 'uncertain':
            split = np.load(f'ensemble_data_split/{ALL_KERNEL[0]}_{FLAGS.uncertainty_metric}.npy')
            li = [split[:FLAGS.transfer_k_shot], [], split[FLAGS.transfer_k_shot:]]
            weight = None
        else:
            li, weight, selected_indices = craig_split(file_li, [train, val, len(dataset)-train-val], 
                                        pragma_dim, model=model, current_train=current_train)
    ...
    if FLAGS.finetune == True:
        if FLAGS.coreset in ['random', 'ensemble_train', 'ensemble_inference']:
            return [train_dataset, val_dataset, test_dataset], li
        elif FLAGS.coreset == 'CRAIG':
            return [train_dataset, val_dataset, test_dataset], weight, li
        else:
            assert FLAGS.coreset == 'CREST'
            return [train_dataset, val_dataset, test_dataset], weight, selected_indices, li
...
def _encode_edge_torch(edge_dict, enc_ftype, enc_ptype):
    ...
    if FLAGS.encode_edge_position:   # This is the case
...
def _coo_to_sparse(coo):
    if not isinstance(coo, coo_matrix):
        coo = coo_matrix(coo)
    assert isinstance(coo, coo_matrix)
    values = coo.data

...
#                'gemver-medium']  # 33 kernels
# Kernels without test kernels
# poly_KERNEL = ['2mm', 'adi', 'atax', 'bicg', 'bicg-large', 'covariance', 'doitgen',
#                'doitgen-red', 'fdtd-2d-large', 'gemm-p', 'gemm-p-large',
#                'gesummv', 'heat-3d', 'jacobi-2d', 'seidel-2d', 'symm', 
#                'symm-opt', 'syrk', 'syr2k', 'trmm', 'mvt-medium', 'correlation',
#                'atax-medium', 'bicg-medium', 'gesummv-medium', 'symm-opt-medium',
#                'gemver-medium']  # 27 kernels
# Test kernels
# poly_KERNEL = ['jacobi-1d', 'fdtd-2d', 'trmm-opt', '3mm', 'gemver', 'mvt']
poly_KERNEL = ['jacobi-1d']
...
# TODO: only when doing evaluation
# target_KERNEL = ["gesummv"]
# poly_KERNEL = target_KERNEL
# MACHSUITE_KERNEL = []
# simple_KERNEL = []
...
# TODO: During regeneration, deactivate the encoder_path to regenerate the encodings
# changed to absolute path
# encoder_path = "/home/weikaili/software-gnn/save/programl/MLP-False-round12-40kernel-icmp-feb-db--regression_edge-position-True_norm_with-invalid_False-normalization_speedup-log2_no_pragma_False_tag_whole-machsuite-poly_perfutil-DSPutil-BRAMutil-LUTutil-FF/encoders.klepto"
...
parser.add_argument('--use_MAML', default=False)  # Only works when subtask = 'train'
parser.add_argument('--finetune', default=transfer_learning)   # Only works when subtask = 'train'
parser.add_argument('--coreset', default='ensemble_inference', choices=['random', 'CRAIG',
                    'ensemble_train', 'ensemble_inference', 'pretrain_test']) # Only works when finetune = True
parser.add_argument('--transfer_k_shot', default=50)
parser.add_argument('--transfer_save_num', default=0)
parser.add_argument('--craig_metric', default='h5', choices=['h5', 'h6', 'h7', 'h8', 'uncertain'])
parser.add_argument('--tsne_plot', default=False)
...
parser.add_argument('--ensemble_KERNEL', default=ensemble_KERNEL)
current_ensemble_KERNEL = ensemble_KERNEL
parser.add_argument('--current_ensemble_KERNEL', default=current_ensemble_KERNEL)
ensemble_model_path = "/home/weikaili/software-gnn/src/logs/auto-encoder/iccad/round1-40kernel/pragma_as_MLP/parallel_and_merge/class/post-gnn-3lp-3lm/dropout-0.1-MSE-scheduler-warmup-weight-decay-0-no-mutual-info-PB-edge-attr-True_position-True-6L-SSL-False-gae-T-False-gae-P-False_regression_train"
parser.add_argument('--ensemble_model_path', default=ensemble_model_path)
parser.add_argument('--ensemble_model_epoch', default=500)
parser.add_argument('--uncertainty_metric', default='low_uncertain', choices=[
                'high_uncertain', 'low_uncertain', 'False', 'hybrid'])
parser.add_argument('--uncertainty_split', default=[45, 5], nargs='+', type=int)
parser.add_argument('--uncertainty_split_alternating', default=False)
parser.add_argument('--ensemble_metric', default='lowest_loss', choices=['average', 'lowest_loss',
                    'weighted', 'similarity_euclidean', 'similarity_cosine'])
...
parser.add_argument('--random_seed', default=4) ## default=100
# TODO: Change it
batch_size = 64
# if graph_type != '':
#     batch_size = 32
...
if not transfer_learning:
    if TASK == 'regression':
        if len(MACHSUITE_KERNEL) + len(poly_KERNEL) > 1: # and v_db == 'v18':
            epoch_num = 1500
        else:
            epoch_num = 500
    else:

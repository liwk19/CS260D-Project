...
class Explorer():
    def __init__(self, path_kernel: str, kernel_name: str, path_graph: str, first_dse: bool = False, run_dse: bool = True, prune_invalid = False, pragma_dim = None):
        ...
        if FLAGS.separate_T and FLAGS.pragma_uniform_encoder:
            if FLAGS.v_db == 'v20': pragma_dim = load(join(dirname(FLAGS.encoder_path), 'v20_pragma_dim'))
            if FLAGS.v_db == 'v21': pragma_dim = load(join(dirname(FLAGS.encoder_path), 'v21_pragma_dim'))
        ...
        if FLAGS.ensemble > 1: ## number of models in ensemble, if 1, not ensemble
            for i in range(FLAGS.ensemble):
                ...
                model = GNNModel(SAVE_DIR, self.log, first_dse = first_dse, multi_target=True, task='regression', num_layers = FLAGS.num_layers, D = FLAGS.D, model_path=model_path, model_id=i, pragma_dim = pragma_dim)
                self.ensemble_GNNmodels.append(model)
        else:
            self.GNNmodel = GNNModel(SAVE_DIR, self.log, first_dse = first_dse, multi_target=True, task='regression', num_layers = FLAGS.num_layers, D = FLAGS.D, pragma_dim = pragma_dim)
        ...
        if self.prune_invalid:
            ...
            ## DAC'22
            self.GNNmodel_valid = GNNModel(SAVE_DIR, self.log, multi_target=False, task='class', num_layers = FLAGS.num_layers, D = FLAGS.D, pragma_dim = pragma_dim) # 6, 64
            #### prev class config ####

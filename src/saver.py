...
class Saver(object):
    def __init__(self, ensemble_base_kernel=None):
        ...
        #base = '/home/weikaili/software-gnn/src/logs'
        ...
        # TODO: change the model saving place
        if len(ALL_KERNEL) == 1:
            if ensemble_base_kernel is not None:
                assert FLAGS.finetune == True and FLAGS.coreset == 'ensemble_train'
                model_specific = '_'.join(model_specific.split('_')[:-1])
                model_specific = model_specific + f'_{ALL_KERNEL[0]}_{ensemble_base_kernel}_{FLAGS.uncertainty_metric}'
                if FLAGS.uncertainty_metric == 'hybrid':
                    model_specific += f'_{FLAGS.uncertainty_split[0]}+{FLAGS.uncertainty_split[1]}_{FLAGS.uncertainty_split_alternating}'
                self.logdir = join(base, model_specific)
            else:
                model_specific = '_'.join(model_specific.split('_')[:-1]) + '_' + ALL_KERNEL[0]

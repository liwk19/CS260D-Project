...
class Net(nn.Module):
    ...
    # TODO: only calculates loss on the valid_indices. It is used by MAML (not implemented yet)
    # TODO: return_middle = i means to return the hidden representation after i GNNs; 0 means no return
    def forward(self, data, valid_indices=None, weight=None, return_middle=0):
            ...
            # for target_name in target_list:
                ...
                if self.task == 'regression':
                    ...
                    if valid_indices is not None:
                        out = out[valid_indices]
                        target = target[valid_indices]
                    if weight is not None:
                        target = target * torch.sqrt(weight)
                        out = out * torch.sqrt(weight)
                    if return_middle == 8:
                        copy_hidden = out.detach().clone()
        ...
        if return_middle > 0:
            return out_dict, total_loss, loss_dict, gae_loss, copy_hidden
        else:
